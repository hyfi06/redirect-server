...
- Click counter: each redirect hit increments an in-memory counter per redirect. When the counter reaches N=10, a `FieldValue.increment(10)` write is sent to Firestore (fire-and-forget — the 302 does not wait). Up to 9 clicks per redirect per instance may be lost on crash; this is accepted for personal analytics.
- `clickCount` field exposed on `GET /api/v1/redirects` and `GET /api/v1/redirects/:id` responses — read-only, absent on documents that have never reached a flush threshold.
- SIGTERM handler in `src/app.js`: flushes all pending counters to Firestore before App Engine shuts down the instance. 10-second timeout; exits 0 on success, 1 on timeout.

### Changed

- `app.yaml`: `min_instances` raised from 0 to 1 so the in-memory counter survives between requests when traffic is low. Cost: 24 instance-hours/day on an F1 instance — within the App Engine free tier.
- Redirect cache now stores `{ id, url }` instead of just `url`, so `clickCounter.increment(id)` can run on cache hits without a second Firestore read.
...
[Unreleased]: https://github.com/hyfi06/redirect-server/compare/v4.2.0...HEAD
[4.2.0]: https://github.com/hyfi06/redirect-server/compare/v4.1.3...v4.2.0
